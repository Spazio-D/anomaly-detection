...
bool readFileCSV(std::string fileName, std::vector<Data> &dataVector) {
    ...
    while (fileCSV.good()) {
        ...
        if ((data.sensorID.length()<=4 || data.sensorID == "SAC10"|| data.sensorID == "SAC11" || data.sensorID == "SAC12" || data.sensorID == "SAC13" || data.sensorID == "SAC14" || data.sensorID == "SAC15") && std::stoi(data.sampleTime)<10){
